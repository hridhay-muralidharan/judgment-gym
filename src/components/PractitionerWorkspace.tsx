...
function RoutineStep({ onNext }: { onNext: () => void }) {
  return <>
    <StepIntro label="01 · Practitioner’s turn" title="Choose what to develop." copy="Maya wants more space before responding to criticism. The practitioner turns that development goal into a short mental workout." />
    <div className="guided-two-column"><div className="guided-panel routine-panel"><div className="guided-panel-top"><span>Workout 01 · Prepared for Maya</span><b>Mental workout</b></div><h4>More room before responding</h4><p className="panel-lede">A sequence for noticing what happens, checking the first story, trying one change, and carrying something useful forward.</p><div className="guided-exercises">{exercises.map(([number, title, faculty, copy]) => <article key={number}><span>{number}</span><div><small>{faculty}</small><strong>{title}</strong><p>{copy}</p></div><em>Included</em></article>)}</div><div className="guided-action-row"><ContinueButton onNext={onNext}>See Maya’s workout</ContinueButton><span>Next: Maya’s turn</span></div></div><aside className="guided-note"><span>Practitioner’s note</span><h4>What to look for</h4><p>Notice whether a pause becomes possible when Maya has more time, support, or language for what she wants to say.</p><dl><div><dt>Maya brings back</dt><dd>A real moment, what changed, and one open question.</dd></div><div><dt>Parts of the mind exercised</dt><dd>Noticing feelings, checking the first story, trying one change, learning.</dd></div></dl></aside></div>
  </>
}

function ClientStep({ onNext }: { onNext: () => void }) {
  return <><StepIntro label="02 · Maya’s turn" title="See how the workout works." copy="Maya knows what she is developing, what she will do, and what she will bring back to the next session." /><div className="guided-client-card"><div><span className="section-label">Maya’s mental workout</span><h4>More room before responding</h4><p>Bring one real moment where criticism changed what you felt able to say or do. The report will stay in your hands before it is shared.</p></div><div className="guided-client-status"><span>Set by practitioner</span><strong>4 exercises</strong></div></div><div className="guided-sequence">{exercises.map(([number, title, faculty, copy]) => <div key={number}><span>{number}</span><div><small>{faculty}</small><strong>{title}</strong><p>{copy}</p></div></div>)}</div><div className="guided-action-row"><ContinueButton onNext={onNext}>See what Maya brings in</ContinueButton><span>Next: a real moment</span></div></>
}
