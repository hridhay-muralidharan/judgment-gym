...
export function Respond({ response, setResponse, scenario, showContext, setShowContext, onSubmit, isReflecting }: Props) {
  return <section className="practice page-grid">
    ...
    <div className="response-column"><div className="response-heading"><p className="kicker">Your reflection</p><span>Private · autosaved</span></div><h3>{scenario.question}</h3><textarea autoFocus value={response} onChange={(event) => setResponse(event.target.value)} placeholder="Write what you notice, feel, and might want to bring to therapy…" /><div className="response-footer"><span>{isReflecting ? 'Finding a careful reflection…' : response.length > 0 ? `${response.length} characters` : 'Take your time'}</span><button className="primary-button" disabled={response.trim().length < 20 || isReflecting} onClick={onSubmit}>{isReflecting ? 'Reflecting…' : 'Show me a mirror'} <span>→</span></button></div></div>
  </section>
}
