...
export function Welcome({ onPrivacy, onPractitioner }: Props) {
  return <section className="landing-page">
    ...
    <div className="landing-section landing-buyer page-grid"><div className="landing-section-title"><p className="kicker">The practitioner’s role</p><h2>Create the workout. Guide the development.</h2></div><div className="landing-section-copy"><p>The practitioner sets the direction, chooses the exercises, and reviews what the client brings back. The client supplies the experience. The relationship gives the workout its purpose.</p></div></div>

    <div className="landing-genai page-grid"><div><p className="kicker">The GenAI layer</p><h2>Now the workout can adapt to the person.</h2></div><div><p>GenAI can follow the client’s words, connect a real situation to a bounded practice variation, and prepare a report that the practitioner can review.</p><p>It helps with the work of adapting the exercise. The practitioner still sets the direction.</p></div></div>
