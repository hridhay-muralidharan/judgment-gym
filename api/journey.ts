...
const schema = {
  ...
  properties: {
    thesis: { type: 'string' },
    periods: { type: 'array', minItems: 2, maxItems: 8, items: { type: 'object', additionalProperties: false, properties: {
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const result = await client.responses.create({
      ...
      input: `Map this private account. Use only the supplied material. Keep the periods chronological where dates or sequence are available. Every evidence field should briefly point to the user's account rather than inventing a citation. The interview narrative should be 100–150 words, first person, and suitable as a draft answer to a product-management interviewer.\n\nPRIVATE ACCOUNT:\n${narrative}`,
