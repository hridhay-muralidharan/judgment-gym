...
export async function requestReflection(scenario: Scenario, response: string, details: HistoryInput['details'], history: HistoryInput[]): Promise<ReflectionResult> {
  const result = await fetch('/api/reflect', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ scenario: `${scenario.title}\n${scenario.body}\n${scenario.context}`, response, details, history }),
  })
